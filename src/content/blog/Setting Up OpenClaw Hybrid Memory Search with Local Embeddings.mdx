...
snippet: "Configure OpenClaw's hybrid memory search using local sentence-transformer embeddings. Set up the all-MiniLM-L6-v2 model, tune vector and text search weights, and enable the embedding cache for fast semantic retrieval."
...
The cache stores computed embeddings so identical queries don't need re-embedding. With `50000` entries and 384-dimensional vectors, the cache uses roughly:

$$50000 \times 384 \times 4\text{ bytes} \approx 73\text{ MB}$$

That's well within the 2 GB RAM of an Azure B2s VM.
