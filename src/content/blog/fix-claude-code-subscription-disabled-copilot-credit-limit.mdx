...
seoTitle: "Fix: Your Organization Disabled Claude Subscription Access"
snippet: "Fix 'Your organization has disabled Claude subscription access for Claude Code'. Use Anthropic API key, Bedrock, or ask admin to enable it."
description: "Fix the 'Your organization has disabled Claude subscription access for Claude Code' error. Switch to an Anthropic API key, Bedrock, Vertex AI, Microsoft Foundry, or ask your admin to enable Claude Code subscription access."
...
tags:
  ...
  - troubleshooting
faq:
  - question: "What does the error 'Your organization has disabled Claude subscription access for Claude Code' mean?"
...
If you searched for **"your organization has disabled Claude subscription access for Claude Code"**, the fix is not to reinstall Claude Code. The organization policy is blocking subscription-based authentication, so use an approved `ANTHROPIC_API_KEY` path or ask the admin to enable Claude Code subscription access.
...
- [Claude API Pricing 2026: Complete Token Costs for Claude 5, Opus 4.8, Sonnet 5, Haiku 4.5](/blog/claude-api-pricing-2026-fable-opus-sonnet-haiku/) — full token pricing tables and cost optimization
- [Hermes Agent Troubleshooting: Fix Model, Provider, Gateway & Credential Errors](/blog/hermes-agent-troubleshooting/) — if your agent errors after switching auth methods
