---
draft: false
title: "Fix OpenClaw Gateway Config Token Mismatch & Port Errors"
snippet: "Fix the OpenClaw gateway token mismatch WebSocket error: why it happens after config changes, restarts, or port conflicts, and how to resync tokens."
description: "Fix the OpenClaw gateway token mismatch WebSocket error: why it happens after config changes, restarts, or port conflicts, and how to resync tokens."
...
### 5. Port Already in Use Forces a Down/Up Cycle

If a previous gateway process didn't release `18789` cleanly, `docker compose restart` can fail with an "address already in use" or "port is already allocated" error. Working around that by running `docker compose down` and `docker compose up -d` instead of a plain `restart` is exactly the recreate path from scenario 4 above — it can regenerate the credentials directory and hand out a new token while old clients still hold the previous one.

```bash
docker compose restart openclaw-gateway
# Error: port is already allocated

# Workaround that can trigger a new token:
docker compose down && docker compose up -d
```
