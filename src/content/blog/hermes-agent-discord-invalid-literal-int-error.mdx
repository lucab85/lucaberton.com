...
**Symptom:** The gateway logs indicate no user-level allowlist is set — the bot connects fine, but every message is treated as unauthorized regardless of who sends it.
...
**Symptom:** After fixing the two issues above, the gateway connects but a new error shows up:

```text
Too many requests. Retry in 276 seconds
```
