...
seoTitle: "Fix OpenClaw 'Non-Loopback Control UI' Startup Error"
snippet: "Fix the OpenClaw startup error 'gateway failed to start: error: non-loopback control ui requires gateway.controlui.allowedorigins (set explicit origins)'."
description: "Fix the OpenClaw startup error 'gateway failed to start: non-loopback control ui requires gateway.controlui.allowedorigins (set explicit origins)' — working."
...
faq:
  - question: 'What causes "non-loopback control ui requires gateway.controlui.allowedorigins"?'
    answer: >-
      It fires when the gateway binds to a network interface such as 0.0.0.0 instead
      of loopback (127.0.0.1) and no allowed origins are listed. OpenClaw requires
      an explicit allowlist before it will expose the Control UI off-host.
  - question: 'How do I fix the non-loopback startup error?'
    answer: >-
      Provide explicit origins and start the gateway again. Add your browser URLs to
      gateway.controlui.allowedorigins in openclaw.json or via openclaw config set,
      then restart the gateway.
  - question: 'Can I bypass the allowedorigins requirement?'
    answer: >-
      You can set gateway.controlui.dangerouslyAllowHostHeaderOriginFallback=true to
      use host-header fallback mode, but it is less secure. Listing explicit origins
      is the recommended fix.
  - question: 'Does this error mean the gateway is running?'
    answer: >-
      No. Unlike "origin not allowed", this is a startup failure — the gateway does
      not start until you provide allowed origins or enable the fallback flag.
---
...
- Hitting the error in your **browser** instead of at startup? See [Fix 'Origin Not Allowed' — OpenClaw Control UI](/blog/openclaw-origin-not-allowed-fix/)
