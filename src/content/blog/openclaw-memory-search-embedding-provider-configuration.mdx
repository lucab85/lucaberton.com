---
draft: false
title: "OpenClaw Memory Search Embedding Provider Setup Guide"
seoTitle: "OpenClaw Memory Search Embedding Provider Setup"
snippet: "Configure OpenClaw memory search with local or API embeddings. Set the memorysearch.provider, choose sentence-transformer models, and tune hybrid weights."
description: "Complete guide to OpenClaw memory search embedding provider configuration. Set up local sentence-transformers or API-based embeddings for semantic memory."
image:
...
### "database is not open" / memory search unavailable

If `memory_search` returns an error like `{"disabled": true, "unavailable": true, "error": "database is not open", ...}`, memory search has not been initialized yet. Work through the same steps as above: confirm an embedding provider is configured, then reindex and restart the gateway.
